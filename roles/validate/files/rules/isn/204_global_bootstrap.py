...
class Rule:
    ...
    def match(cls, data_model):
        ...
        if dhcp:
            ...
            dhcp_block = cls.safeget(data_model, ['vxlan', 'multisite', 'isn', 'bootstrap', dhcp])
            if isinstance(dhcp_block, dict) and dhcp_block.get('multi_subnet_scope'):
                ...
                results.extend(
                    cls.validate_multi_subnet_scope(
                        ...
                        version, prefix_min, prefix_max,
                        f"vxlan.multisite.isn.bootstrap.{dhcp}.multi_subnet_scope",
                    )
                )
